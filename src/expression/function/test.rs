    ...
    mod test_nesting_properties {
        ...
        fn it_allows_nesting() {
            ...
            let pythag = SimplexFunction::new("Pythag")
                .push_meta_variable(SimplexAtom::from("a"))
                .push_meta_variable(SimplexAtom::from("b"))
                .push(&SimplexPointer::from(SimplexFunction::new("Plus")
                    .toggle_reflexive()
                    .push_meta_variable(SimplexAtom::from("a"))
                    .push_meta_variable(SimplexAtom::from("b"))
                    .push(&SimplexPointer::from(pow.clone().evaluate(&vec!["a", "2"])))
                    .push(&SimplexPointer::from(pow.clone().evaluate(&vec!["b", "2"]))).evaluate(&vec!["a", "b"])))
                .toggle_reflexive();
